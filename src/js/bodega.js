...
// ════════════════════════════════════════════════════════════════════
let acapararCache = { escuelas: null, stock: null, prendaActual: null };
...
async function abrirAcapararModal() {
  ...
  modal.style.display = 'flex';
  try {
    if (!acapararCache.escuelas) {
      acapararCache.escuelas = await supaFetchAll('escuela',
        '?activa=eq.true&select=id,alias,nombre,codigo_cde&order=alias');
    }
    acapararCache.stock = await supaFetchAll('vw_bodega_stock',
      '?stock_actual=gt.0&select=cod_prenda,nombre_prenda,talla_key,stock_actual');
    const acaDisp = await supaFetchAll('vw_acaparado_disponible',
      '?select=escuela_id,nombre_prenda,talla_key,disponible');
    acapararCache.acaparadoAjeno = acaDisp;  // todo el acaparado (de cualquier escuela)

    const selE = document.getElementById('aca-escuela');
    selE.innerHTML = '<option value="">— Elegí escuela —</option>' +
      acapararCache.escuelas.map(e => `<option value="${e.id}">${e.alias || e.nombre}${e.codigo_cde ? ' · '+e.codigo_cde : ''}</option>`).join('');

    const prendas = [...new Set(acapararCache.stock.map(s => s.nombre_prenda || s.cod_prenda))].sort();
...
function onAcapararTallaCambio() {
  ...
  // Acaparado total de TODAS las escuelas para este prenda+talla
  ...
  info.innerHTML = `
    ...
    · Acaparado por otras escuelas <strong>${acapTotal}</strong>
