  ...
  cont.innerHTML = header + `
    <div class="card" style="padding:0;overflow:hidden">
      <div style="display:flex;gap:6px;align-items:center;padding:8px 12px;font-size:11px;color:#888;flex-wrap:wrap">
        <span style="flex:1;min-width:150px">${grupos.size} grupo(s) · ${lista.length.toLocaleString()} alumno(s) · tap grupo = abrir · tap alumno = ficha</span>
...
// Filas ultra-compactas de un grupo plegable (tap = ficha completa)
function agFilasGrupo(arr) {
  const talla = (a, esTop) => {
    ...
    if (!pedida) return '<span style="color:#c44;min-width:40px;text-align:center">⚠</span>';
    return `<span style="font-family:monospace;font-weight:600;color:var(--azul);min-width:40px;text-align:center;${esAlt ? 'background:#FFF4CC;outline:1px dashed #C90;outline-offset:-1px;border-radius:3px;padding:0 3px' : ''}">${show}${esAlt ? `<s style="font-size:9px;color:#888;margin-left:2px">${pedida}</s>` : ''}</span>`;
  };
  const ic = (e) => e === 'empacado' ? '✅' : (e === 'entregado' ? '🚚' : '⬜');
  return arr.map(a => `
    <div onclick="editarAlumnoRapido('${a.id}')" title="Abrir ficha"
      style="display:flex;align-items:center;gap:8px;padding:8px 12px;border-top:1px solid #F0F0F0;cursor:pointer;${alumnoSinTallas(a) ? 'background:#FFF9F7' : ''}">
