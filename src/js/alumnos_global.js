...
async function generarPdfDirecto(html, filename) {
  ...
  // 1) Overlay de progreso
  const overlay = document.createElement('div');
  overlay.id = 'pdf-progress-overlay';
  overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9999;display:flex;align-items:center;justify-content:center;color:white;font-size:18px;text-align:center;padding:20px';
  overlay.innerHTML = `
    <div>
  ...
  // 2) Container ON-screen pero invisible. html2canvas necesita el layout
  //    realmente renderizado, no off-screen con coords negativas.
  const wrap = document.createElement('div');
  wrap.id = 'etiq-pdf-wrap';
  wrap.style.cssText = 'position:fixed;left:0;top:0;width:8.5in;background:white;visibility:hidden;z-index:-1;pointer-events:none;';
  // Extraer body y estilos
  const m = html.match(/<body[^>]*>([\s\S]*?)<\/body>/i);
  const bodyHtml = m ? m[1] : html;
  const sm = html.match(/<style[^>]*>([\s\S]*?)<\/style>/i);
  if (sm) {
    const styleEl = document.createElement('style');
    styleEl.textContent = sm[1].replace(/@page[^}]*\}/g, '');
