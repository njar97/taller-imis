...
// Inserta el HTML en un contenedor oculto del documento actual,
// genera el PDF con html2pdf y lo descarga. Sin ventana nueva.
async function generarPdfDirecto(html, filename) {
  // 1) Container temporal off-screen (no display:none — html2canvas necesita layout)
  const wrap = document.createElement('div');
  wrap.id = 'etiq-pdf-wrap';
  wrap.style.cssText = 'position:absolute;left:-99999px;top:0;width:8.5in;background:white;';
  // Extraemos solo el body content del template
  const m = html.match(/<body[^>]*>([\s\S]*?)<\/body>/i);
  const bodyHtml = m ? m[1] : html;
  // Extraemos también el <style> para inyectarlo
  const sm = html.match(/<style[^>]*>([\s\S]*?)<\/style>/i);
  if (sm) {
    const styleEl = document.createElement('style');
    styleEl.textContent = sm[1].replace(/@page[^}]*\}/g, ''); // @page no aplica fuera de print
    wrap.appendChild(styleEl);
  }
  wrap.insertAdjacentHTML('beforeend', bodyHtml);
  // Quitar la botonera (.no-print) — no debe ir en el PDF
  wrap.querySelectorAll('.no-print').forEach(el => el.remove());
  document.body.appendChild(wrap);

  // 2) Auto-fit cada celda con [data-fit-max]
  try {
  ...
  // 3) Verificar que html2pdf esté disponible
...
function renderHojaEtiquetas(grupos, cols, incluirObs, escuelasMap, tempCodeHdr, totalAlumnos) {
  ...
  const renderUnaEtiqueta = (a) => {
    ...
    return `
      <div class="etiqueta">
        ...
        <span class="tallas" data-fit-max="18" data-fit-min="9"><b>${top}</b>&nbsp;<b>${bot}</b></span>
      </div>
    `;
  };
  ...
  return `
    ...
    <style>
      ...
      /* Columnas FIJAS en mm. Total ancho usable (Letter - márgenes) ≈ 200mm.
         Distribución: grado 14 + sexo 7 + nombre 90 + centro 45 + tallas 32 + gaps. */
      .etiqueta {
        height: 10mm;
        border-top: 0.5pt dashed #888;
        padding: 0 1mm;
        display: grid;
        grid-template-columns: 14mm 7mm 90mm 45mm 32mm;
        column-gap: 2mm;
        align-items: center;
        overflow: hidden;
