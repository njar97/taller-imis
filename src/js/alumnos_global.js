...
function abrirModalEtiquetas() {
  ...
  if (!modal) return;
  const c = alumnosGlobalCache;
  // Multiselect de escuelas: cargar con todas las que tienen alumnos
  ...
  if (selMulti) {
    ...
    // Si hay un filtro activo en la vista, pre-seleccionar esa escuela; sino "Todas".
    const preFiltro = c.filtroEscuela;
    selMulti.innerHTML = `<option value="__all__" ${!preFiltro?'selected':''}>Todas las escuelas</option>` +
      opts.map(e => `<option value="${e.id}" ${e.id===preFiltro?'selected':''}>${e.alias ? e.alias + ' · ' : ''}${e.nombre}</option>`).join('');
  }
...
function generarEtiquetas() {
  // Escuelas seleccionadas (multi). Si está "__all__", no filtramos.
  const selMulti = document.getElementById('et-escuelas-multi');
  const seleccionadas = selMulti ? Array.from(selMulti.selectedOptions).map(o => o.value) : ['__all__'];
  const todasLasEscuelas = seleccionadas.length === 0 || seleccionadas.includes('__all__');
  const escuelasFiltro = todasLasEscuelas ? null : new Set(seleccionadas);

  // Temporada: siempre la activa
