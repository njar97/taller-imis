...
function renderAlumnosGlobal() {
  ...
  const filas = visible.map(a => {
    ...
    const topElig = (c.modoEmpaque && setP && setP.has(a.prenda_top) && a.talla_top_key
      && a.estado_top !== 'empacado' && a.estado_top !== 'entregado');
    const botElig = (c.modoEmpaque && setP && setP.has(a.prenda_bottom) && a.talla_bottom_key
      && a.estado_bottom !== 'empacado' && a.estado_bottom !== 'entregado');
    ...
    const checkCell = c.modoEmpaque
      ? `<td style="padding:4px 8px;text-align:center" onclick="event.stopPropagation()">${topCheck}</td>
         <td style="padding:4px 8px;text-align:center" onclick="event.stopPropagation()">${botCheck}</td>`
      : '';
