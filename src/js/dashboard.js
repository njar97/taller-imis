...
async function cargarDashboardHoy() {
  ...
  try {
    const [
      ...
      cargarProduccionHoy(),
      // Necesito escuela_id + estados para construir el embudo por escuela
      supaFetchAll('alumno', '?activo=eq.true&select=escuela_id,estado_top,estado_bottom,talla_top_key,talla_bottom_key').catch(() => []),
      supaFetchAll('escuela', '?activa=eq.true&select=id,alias,nombre,codigo_cde&order=alias').catch(() => []),
      supaFetchAll('escuela_acaparado', '?select=escuela_id,cantidad_acaparada,cantidad_consumida').catch(() => []),
    ]);
