...
async function exportarHojaEntregaPDF() {
  ...
  try {
    ...
    // 3) Contrato vs entregado por NIVEL educativo (usando pedido)
    const porNivel = new Map();  // nivel → { contratado, entregado }
    for (const p of pedidos) {
      const niv = p.nivel || 'OTRO';
      if (!porNivel.has(niv)) porNivel.set(niv, { contratado: 0, entregado: 0 });
      const o = porNivel.get(niv);
      o.contratado += Number(p.cantidad_solicitada) || 0;
      o.entregado  += Number(p.cantidad_entregada)  || 0;
    }
