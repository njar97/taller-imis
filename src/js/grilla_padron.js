...
const GP_COLS = [
  { k: 'nombre',     label: 'Nombre',  edit: true,  min: '160px', get: a => a.nombre || '' },
  { k: 'grado',      label: 'Grado',   edit: true,  min: '64px',  get: a => a.grado || '' },
  { k: 'sexo',       label: 'Sexo',    edit: true,  min: '44px',  get: a => a.sexo || '' },
  { k: 'escuela',    label: 'Escuela', edit: false, min: '80px',
    get: a => { const e = alumnosGlobalCache.escuelas[a.escuela_id]; return e ? (e.alias || e.nombre) : ''; } },
  { k: 'prenda_top', label: 'Prenda top', edit: false, min: '96px', get: a => a.prenda_top || '' },
  { k: 'talla_top',  label: 'T. top',  edit: true,  min: '52px',  get: gpTallaTop },
  { k: 'prenda_bot', label: 'Prenda bot', edit: false, min: '96px', get: a => a.prenda_bottom || '' },
...
function renderGrillaPadron(cont, header, lista) {
  ...
  const totalPaginas = Math.max(1, Math.ceil(lista.length / GP_PAGINA));
  if (c.pagina > totalPaginas) c.pagina = totalPaginas;
  const inicio = (c.pagina - 1) * GP_PAGINA;
  const visible = lista.slice(inicio, inicio + GP_PAGINA);
  ...
  cont.innerHTML = header + `
    <div class="card" style="padding:8px 10px;margin-bottom:10px;display:flex;gap:8px;align-items:center;flex-wrap:wrap">
      ...
      <span style="font-size:11px;color:#aaa;flex-basis:100%">
        ✏️ Click en una celda para editar · Enter ↓ · Tab → · pegá una columna de Excel y rellena hacia abajo</span>
    </div>
      ...
      <div style="flex:1;min-width:0">
        <div class="card" style="padding:0;overflow-x:auto">
          <table id="gp-tabla" style="border-collapse:collapse;width:100%;font-size:13px">
            <thead><tr>
              ${GP_COLS.map(col => `<th style="position:sticky;top:0;background:#F5F7FA;border-bottom:2px solid var(--borde);padding:6px 8px;text-align:left;min-width:${col.min};white-space:nowrap">${col.label}</th>`).join('')}
            </tr></thead>
            <tbody>${filas}</tbody>
          </table>
        </div>
