...
async function exportarTallasPDF() {
  ...
  function buildTablaGrupo(grupoKey, groupRows, isFirst) {
    ...
    const subtitulo = isCombi
      ? `<h2 style="margin:10px 0 6px;font-size:18pt;color:#1F4E79;font-weight:700">👗 FALDA + FALDA C.E <span style="font-size:11pt;font-weight:400;color:#666">(mezcladas, ordenadas por talla y largo · tallas con prefijo <strong>FCE</strong> = con elástico)</span></h2>`
    ...
    // (ej. FCE635) y las FALDA normales con "F" (ej. F635), así que no
    // hace falta sufijo extra para distinguirlas — el código ya lo indica.
    ...
    // Grupos chicos (≤8 filas) envueltos con page-break-inside:avoid
    // para que NO se rompan a media tabla — quedan compactos en la
    // página que estén. Para los grandes el browser pagina natural.
    const isSmall = groupRows.length <= 8;
    const wrapStyle = isSmall ? 'page-break-inside:avoid' : '';
    // Si es chiquito, no forzar page-break-before (deja que el browser
    // lo acomode al final de la página anterior si hay espacio).
    const pageBreak = (isFirst || isSmall) ? '' : '<div style="page-break-before:always"></div>';

    return `${pageBreak}<div style="${wrapStyle}">${subtitulo}<table style="width:100%;border-collapse:collapse;font-family:Arial,sans-serif;table-layout:fixed">${cg}<thead><tr>${hdr}</tr></thead><tbody>${dr}${tr}</tbody></table></div>`;
  }
...
<style>
  @page { size: letter ${orientacion}; margin: 0.25in; }
...
<body>${html}</body>
  ...
  // Throttle suave: si tocó hace menos de 1s, ignorar (evita doble-tap).
  // Usa timestamp en vez de boolean para que NO se quede pegado si algo falla.
  const now = Date.now();
  if (now - (window._tallasPdfLastTry || 0) < 1000) {
    console.warn('[tallas PDF] throttle (doble-tap), ignorando');
    return;
  }
  window._tallasPdfLastTry = now;

  let iframe = null;
  try {
    // Iframe usando srcdoc — más confiable en PWA standalone que el
    // approach document.write(), que en algunos contextos (Chrome
    // Android instalado como PWA) puede no triggerear el evento load.
    iframe = document.createElement('iframe');
    iframe.style.cssText = 'position:fixed;right:0;bottom:0;width:0;height:0;border:0;visibility:hidden';
    iframe.setAttribute('aria-hidden', 'true');
    iframe.setAttribute('sandbox', 'allow-same-origin allow-modals allow-scripts');
    // Esperar load ANTES de inyectar srcdoc — así no perdemos el evento.
    ...
    console.log('[tallas PDF] iframe ready via:', reason);

    // Pausa extra para que fuentes terminen de aplicar
    await new Promise(r => setTimeout(r, 250));

    if (!iframe.contentWindow) throw new Error('contentWindow no disponible — sandbox o navegación bloqueó el iframe');
    iframe.contentWindow.focus();
    iframe.contentWindow.print();
    console.log('[tallas PDF] print() OK');
  } catch (e) {
    console.error('[tallas PDF] error:', e);
    // Fallback: abrir ventana nueva con el HTML para que el usuario
    // pueda usar el menú del navegador → Imprimir manualmente.
    try {
      ...
      if (win) {
        ...
        alert('No se pudo abrir el PDF (' + (e && e.message || e) +
              ').\n\nProbá tocar el botón otra vez. Si sigue sin funcionar, ' +
              'cerrá y reabrí la app desde el ícono.');
      }
    } catch (e2) {
      alert('Error al generar el PDF: ' + (e && e.message || e));
    }
  } finally {
    // Remover iframe después de 30s — algunos browsers cancelan la
    // impresión si se quita antes de que el usuario complete el flujo.
    setTimeout(() => {
