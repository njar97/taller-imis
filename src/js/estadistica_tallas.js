...
async function exportarTallasPDF() {
  ...
  if (showEsc && esc.length > 0) {
    ...
    else for (let i = 0; i < esc.length; i += CHUNK_ESC) escChunks.push(esc.slice(i, i + CHUNK_ESC));
  } else {
    escChunks.push([]);  // una sola tabla sin desglose por escuela
  }
  ...
  function buildTablaChunk(chunkEsc, chunkIdx, totChunks) {
    ...
    const cg = `<colgroup>
      ...
      ${showEscChunk ? chunkEsc.map(() => `<col style="width:${escW.toFixed(3)}%">`).join('') : ''}
    ...
    const hdr = `
      ...
      ${showEscChunk ? chunkEsc.map(e => `<th style="${thS}" title="${(e.nombre||'').replace(/"/g,'&quot;')}">🏫 ${e.alias || e.nombre}</th>`).join('') : ''}
    ...
    const dr = rows.map((r, i) => {
      ...
      return `<tr style="${bg}">
        ...
        ${showEscChunk ? chunkEsc.map(e => {
          const n = r.porEsc.get(e.id) || 0;
          return `<td style="${tdS};text-align:center;color:${n>0?'#222':'#CCC'}">${n||'·'}</td>`;
        }).join('') : ''}
    ...
    const tr = `<tr style="background:#E8F0FE;font-weight:700">
      ...
      ${showEscChunk ? chunkEsc.map(e => {
        ...
        return `<td style="${tdS};text-align:center;font-weight:700">${sum||'·'}</td>`;
      }).join('') : ''}
    ...
    const subtituloChunk = (totChunks > 1 && showEscChunk)
      ? `<div style="margin:8px 0 4px;font-size:12px;font-weight:600;color:#1F4E79">📍 Bloque ${chunkIdx+1}/${totChunks} · Escuelas: ${chunkEsc.map(e => e.alias || e.nombre).join(', ')}</div>`
      : '';
    const pageBreak = chunkIdx > 0 ? '<div style="page-break-before:always"></div>' : '';

    return `${pageBreak}${subtituloChunk}<table style="width:100%;border-collapse:collapse;font-family:Arial,sans-serif;table-layout:fixed">${cg}<thead><tr>${hdr}</tr></thead><tbody>${dr}${tr}</tbody></table>`;
  }

  const tablasHtml = escChunks.map((ch, i) => buildTablaChunk(ch, i, escChunks.length)).join('');
