...
async function exportarTallasPDF() {
  ...
  function buildTablaGrupo(grupoKey, groupRows, isFirst) {
    ...
    // Detectar si una fila es FALDA_C.E (con elástico). Usado solo en
    // el grupo combinado para anexar "(CE)" en la celda Talla.
    const esCE = (prenda) => isCombi && prenda && !/^FALDA$/i.test(String(prenda).trim());

    const dr = groupRows.map((r, i) => {
      const bg = i % 2 === 0 ? 'background:white' : 'background:#FAFAFA';
      const balColor = r.balance < 0 ? '#C00' : '#2a8f4a';
      const ceTag = esCE(r.prenda) ? ` <span style="font-size:9pt;color:#1F4E79;font-weight:600">(CE)</span>` : '';
      return `<tr style="${bg}">
