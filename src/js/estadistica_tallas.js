...
let tallasResumenCache = {
  ...
  pool: [],
  filtros: {
    prenda: '',       // string single
    talla: '',        // string single
    escuelas: [],     // multi
    soloFaltantes: false,  // mostrar solo filas con balance negativo
    incluirCorte: true,
    incluirProd: true,
    incluirBodega: true,
    incluirPool: true,
...
async function initTallasResumen() {
  ...
  try {
    ...
    const [escuelas, alumnos, bultosPend, stock, pool] = await Promise.all([
      ...
      // Bultos en producción no terminados: pendientes o en proceso. Filtro por estado != terminado.
...
function renderTallasResumen() {
  ...
  // ─── Computar demanda por (prenda, talla, escuela) ────────────────
  // Una pieza por alumno cuya talla matchea y estado != empacado/entregado
  ...
  // Pool por (prenda, talla, escuela): disponible = acaparada - consumida
  // Y total pool por (prenda, talla)
  const poolTotal = new Map();
  const poolPorEsc = new Map();   // "prenda|talla" → Map<eid, disponible>
  ...
  // ─── Combinar todas las keys que aparecen en cualquier lado ───────
  ...
  for (const k of allKeys) {
    ...
    // Filtro por escuelas: si hay alguna escuela elegida, solo mostrar filas donde
    // al menos una de ellas tenga demanda > 0
    if (f.escuelas && f.escuelas.length > 0) {
      const matchEsc = f.escuelas.some(eid => (dem.porEsc.get(eid) || 0) > 0);
      if (!matchEsc) continue;
    }
    const corte = 0;  // por ahora no separamos corte; queda 0
    ...
    if (f.soloFaltantes && balance >= 0) continue;
    rows.push({
      key: k, prenda, talla,
  ...
  root.innerHTML = `
    ...
    <div class="card" style="padding:10px;margin-bottom:10px">
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:8px;margin-bottom:8px">
        <div class="field" style="margin:0">
          <label>Prenda</label>
          <select onchange="onTallasFiltro('prenda', this.value)">
            <option value="">Todas</option>
            ${prendasUnicas.map(p => `<option value="${p}" ${f.prenda===p?'selected':''}>${p}</option>`).join('')}
          </select>
        </div>
        <div class="field" style="margin:0">
          <label>Talla</label>
        ...
        <div class="field" style="margin:0">
          <label style="display:flex;gap:6px;align-items:center;font-weight:normal">
            <input type="checkbox" ${f.soloFaltantes?'checked':''} onchange="onTallasFiltro('soloFaltantes', this.checked)">
            Solo faltantes
          </label>
        </div>
    ...
    <div class="card" style="padding:0;overflow:hidden">
      <div style="background:#F5F7FA;padding:8px 12px;font-weight:600">📊 Detalle por talla</div>
      <div style="overflow-x:auto">
        <table style="width:100%;border-collapse:collapse;font-size:12px;min-width:780px">
          ...
          <tbody>
            ${rows.length === 0 ? `
              <tr><td colspan="9" style="padding:20px;text-align:center;color:#888">Sin resultados con los filtros aplicados.</td></tr>
            ` : rows.map(r => {
    ...
    <div style="text-align:center;margin-top:8px;color:#888;font-size:11px">
      ${rows.length} fila(s). Tocá una fila para desglosar por escuela.
    </div>
  `;
}
